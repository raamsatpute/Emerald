/****** Object:  StoredProcedure [dbo].[spMetalMetrics_ByType]    Script Date: 6/18/2020 11:53:37 PM ******/
SET ANSI_NULLS ON
GO
...
-- Description:	This SP Fetch Metal Pounds/Revenue as per ItemId and as per value passed in below parameter
...
--exec spMetalMetrics_ByType '1',2019,10,'','','Summary'
--exec spMetalMetrics_ByType '1,13',2019, 11, '','SSINI,ALCOR,MIXCOR','Detail'--For Monthly Detail and with selected Item
--exec spMetalMetrics_ByType '1,13',2019, 11, '','SSINI,ALCOR,MIXCOR','Detail' --For Monthly Detail and for all Item (If none of Item selected in Main Filter than all items else selected items)
--exec spMetalMetrics_ByType '1,13',2019, 11, '','ALCOR','GrandTotal' --For Yearly Detail and with selected Item
--exec spMetalMetrics_ByType '1,13',2019, 11, '','ALCOR,MIXCOR','GrandTotal' --For Yearly Detail and for all Item (If none of Item selected in Main Filter than all items else selected items)
--exec spMetalMetrics_ByType '1,13',2019, 11, '','ALCOR,MIXCOR','GrandTotal' --For Yearly Detail and for all Item (If none of Item selected in Main Filter than all items else selected items)

CREATE PROCEDURE [dbo].[spMetalMetrics_ByType]
...
BEGIN
		...
		Begin
			...
			)a group by ItemId, Entity, MetalPeriod, fiscalyear,fiscalperiod
			...
			SELECT @query = 
				...
				(
					SELECT      
						ItemId, Entity,	MetalPeriod, PoundAmt
					FROM #arQtyData
				)X
				PIVOT 
				(   
