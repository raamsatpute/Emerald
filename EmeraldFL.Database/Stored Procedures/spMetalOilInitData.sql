...
/****** Object:  StoredProcedure [dbo].[spMetalOilInitData]    Script Date: 6/18/2020 11:35:08 PM ******/
SET ANSI_NULLS ON
GO
...
--				3.	Item
-- =============================================
--exec spMetalOilInitData  '14,15'
--exec spMetalOilInitData  '14,15'

alter PROCEDURE [dbo].[spMetalOilInitData]
...
BEGIN
	...
	-------------------------Item Start---------------------------
	IF ( EXISTS ( select distinct partid from trav_EO_ArDetailHistory where catid like 'fg%' and partid  is not null and partid <>'') )
    select distinct partid from trav_EO_ArDetailHistory where catid like 'fg%' and partid  is not null and partid <>'' order by partid
	ELSE
