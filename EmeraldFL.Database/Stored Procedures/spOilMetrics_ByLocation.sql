...
/****** Object:  StoredProcedure [dbo].[spOilMetrics_ByLocation]    Script Date: 6/18/2020 11:58:17 PM ******/
SET ANSI_NULLS ON
GO
...
-- Description:	This SP Fetch Metal Pounds/Revenue as per Location and as per value passed in below parameter
...
-- =============================================
--exec spOilMetrics_ByLocation '1,7,8',2019,10,'','','Summary',''
--exec spOilMetrics_ByLocation '1,7,8,9,10,11,12,13,14',2019,10,'','','Summary',''
--exec spOilMetrics_ByLocation '1,13',2019, 11, '','','Detail','Florida' --For Monthly Detail and with selected Item
--exec spOilMetrics_ByLocation '1,8',2019, 11, '','','Detail','Arizona' --For Monthly Detail and for all Item (If none of Item selected in Main Filter than all items else selected items)
--exec spOilMetrics_ByLocation '1,13',2019, 12, '','OIL249,OILFR3','GrandTotal','Florida' --For Yearly Detail and with selected Item
--exec spOilMetrics_ByLocation '',2019, 12, '','OIL249,OILFR3','GrandTotal','' --For Yearly Detail and for all Item (If none of Item selected in Main Filter than all items else selected items)
--exec spOilMetrics_ByLocation '',2019, 12, '','OIL249,OILFR3','GrandTotal','' --For Yearly Detail and for all Item (If none of Item selected in Main Filter than all items else selected items)

create PROCEDURE [dbo].[spOilMetrics_ByLocation]
...
BEGIN
		...
		Begin
			...
			)a group by Location, Entity, OilPeriod, fiscalyear,fiscalperiod
			...
			SELECT @query = 
				'SELECT Location, Entity'+ @NullToZeroCol +' into tmpAmtMonth FROM
				(
					SELECT      
						Location, Entity,	OilPeriod, GallonAmt
					FROM #arQtyData
				)X
				PIVOT 
				(   
			...
			SELECT @query = 
			'Select Location, Entity' + @NullToZeroCol + ', ' + @colSum + ' as GrandTotal into tmpAmtTotal from tmpAmtMonth tm' 

			EXEC SP_EXECUTESQL @query

			SELECT @query = 
			'Select Location, Entity' + @NullToZeroCol + ', ' + 'GrandTotal, GrandTotal/12 as Average
			 from tmpAmtTotal order by Location, Entity'
		...
		Else If @Key = 'Detail'
		Begin
			--select @LocationId=Id from Locations where (active=1  or id=14)and Description=@Filter
		...
		select @LocationId=Id from Locations where (active=1  or id=14)and Description=@Filter
			...
			Case WHEN @PlantCode IS NULL THEN 1
				WHEN ar.WhseId IN (SELECT FIELDVALUE FROM FN_STRING_TO_TABLE_STRING(@PlantCode, ',')) THEN 1 ELSE 0 END = 1
				--and l.[Description]=isnull(@Filter,'')
