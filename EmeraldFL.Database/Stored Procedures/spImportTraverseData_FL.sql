/****** Object:  StoredProcedure [dbo].[spImportTraverseData_FL]    Script Date: 08/14/19 10:18:20 AM ******/
SET ANSI_NULLS ON
GO
	...
	Begin Try
		...
		Update targetUnitData
			SET	targetUnitData.[Job Type] = sourceExportJobs.[Job Type], targetUnitData.[KVA] = sourceExportJobs.[KVA],
				...
				targetUnitData.[ProcessDate] = case when ISDate(sourceExportJobs.[ProcessDate]) = 1 then sourceExportJobs.[ProcessDate] else null end,
				targetUnitData.[CarcassManifestDate] = case when ISDate(sourceExportJobs.[CarcassManifestDate]) = 1 then sourceExportJobs.[CarcassManifestDate] else null end,
		...
		Begin Try
			...
			Select 
				...
				case when ISDate(sourceExportJobs.[ProcessDate]) = 1 then sourceExportJobs.[ProcessDate] else null end,
				case when ISDate(sourceExportJobs.[CarcassManifestDate]) = 1 then sourceExportJobs.[CarcassManifestDate] else null end
			FROM @traverse_ExportJobs as sourceExportJobs 
