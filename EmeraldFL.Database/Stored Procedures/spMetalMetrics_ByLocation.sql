/****** Object:  StoredProcedure [dbo].[spMetalMetrics_ByLocation]    Script Date: 6/18/2020 11:52:10 PM ******/
SET ANSI_NULLS ON
GO
...
--exec spMetalMetrics_ByLocation '8',2019,12,'','','Summary',''
--exec spMetalMetrics_ByLocation '1',2019, 12, '','ALCOR,CUCOR,CUNUG,CUWIND,MIXCOR,SSINI','Detail', ''--For Monthly Detail and with selected Item
--exec spMetalMetrics_ByLocation '1,13',2019, 11, 'Field D','SSINI,ALCOR,MIXCOR','Detail','Florida,Mississippi' --For Monthly Detail and for all Item (If none of Item selected in Main Filter than all items else selected items)
--exec spMetalMetrics_ByLocation '1,13',2019, 11, '','ALCOR','GrandTotal','Florida' --For Yearly Detail and with selected Item
--exec spMetalMetrics_ByLocation '1,13',2019, 11, '','','GrandTotal','' --For Yearly Detail and for all Item (If none of Item selected in Main Filter than all items else selected items)
--exec spMetalMetrics_ByLocation '1,13',2019, 11, '','','GrandTotal','' --For Yearly Detail and for all Item (If none of Item selected in Main Filter than all items else selected items)

create PROCEDURE [dbo].[spMetalMetrics_ByLocation]
...
BEGIN
		...
		Begin
			...
			)a group by Location, Entity, MetalPeriod, fiscalyear,fiscalperiod
			...
			SELECT @query = 
				...
				(
					SELECT      
						Location, Entity,	MetalPeriod, PoundAmt
					FROM #arQtyData
				)X
				PIVOT 
				(   
